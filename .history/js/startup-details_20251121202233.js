document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        // Features - FIX FOR KEY FEATURES & IMPACT
        ...
        // FIX FOR PHONE NUMBER: Ensure the container is visible if data exists
        ...
        if(data.phone) {
            ...
            // Assuming your CSS hides it by default if empty, we force it to show.
            // If phoneContainer is being dynamically hidden, you'll need to override that.
            if (phoneContainer) {
                // Set to 'flex' as it's an icon row
                phoneContainer.style.display = 'flex'; 
            }
        } else {
             // If there's no phone number, explicitly hide the container (if it's not already)
            if (phoneContainer) {
